...
async function akka(req, res) {
    if (req.user) {
        res.status(200).json({ "msg": `im portected and you managed to reach me ${req.user}` });
    }
    else {
...
async function register(req, res) {
    ...
    if (!errorResult.isEmpty()) {
        return res.status(400).json({ "ValidationError": "All Fields Required", errorResult }); // 400 bad request
    }
    try {
...
async function Emailvalidation(req, res) {
    try {
        ...
        await User.findOneAndUpdate({ _id: decoded.userId }, { emailConfirmed: true });
...
async function login(req, res) {
    ...
    try {
        ...
        if (!user.emailConfirmed) {
            res.status(400).json({ error: "Please confirm your email addresss to login!" });
        }
        // found a user  create an httpONly cookie
        const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET_TOKEN, { expiresIn: "2m" });
...
async function resetPass(req, res) {
    res.json({ "msg": "resetPass" });
};

export default { register, login, logout, forgetPass, resetPass, Emailvalidation, akka };
