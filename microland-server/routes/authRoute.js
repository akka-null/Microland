...
router.post('/register',
    body('username').trim().escape().custom(async (name) => {
        const oldName = await User.findOne({ username: name });
        if (oldName) {
            throw new Error('Username already exists');
        }
    }),
    body('email').trim().escape().isEmail().withMessage('Please use a valid E-mail address').custom(async (mail) => {
        const oldEmail = await User.findOne({ email: mail })
        if (oldEmail) {
            throw new Error('E-mail already in use');
        }
    }),
    body('password', 'please use a password at least with 5 characters').isLength({ min: 5 }),
...
// login 
router.post('/login',
    body('email').trim().escape().isEmail().withMessage('Please use a valid E-mail address'), // email validation / sanitazing 
