    ...
    <div v-for="prod in store.products" :key="prod._id">
        <div class="rounded-xl border bg-card text-card-foreground shadow">
            ...
            <a @click="goToProductPage(prod._id)" class="relative flex h-60 overflow-hidden" >
                <img @click="goToProductPage(prod._id)" class="absolute top-0 right-0 h-full w-full object-cover" v-bind:src="prod.images[0]" alt="product image" />
            </a>
            ...
            <div class="mt-4 px-5 pb-5">
                ...
                <button @click="store.addToCart(prod)" class="flex items-center justify-center bg-gray-900 px-2 py-1 text-sm text-white transition hover:bg-gray-700">
