...
export const authStore = defineStore('user', {
    ...
    actions: {
       ...
       async signUp(username: string, email: string, password: string, confirm: string) {
            try {
                const res = await axios.post(`${BASE_URL}/api/register`, {username, email, password, confirmPassword: confirm});
                console.log(res);
                return {
                    message: res.data.msg,
                    status: 200,
        ...
        async reset(password: string, confirmPassword: string, token: string) {
            try {
                const update = await axios.post(`${BASE_URL}/api/reset/${token}`, { password, confirmPassword });
                return {
