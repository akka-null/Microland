...
export const productStore = defineStore('products', {
  ...
  actions: {
    getProductById(id: string) {
      axios.get(`${BASE_URL}${PRODUCT_URL}/${id}`).then((item) => {
        this.product = item.data.product
      })
    },
    getProducts(page: number, itemPerPage: number) {
      axios
        .get(`${BASE_URL}${PRODUCTS_URL}/?page=${page}&itemPerPage=${itemPerPage}`)
        .then((items) => {
    ...
    getProductsByType(page: number, itemPerPage: number, type: string) {
      axios
        .get(`${BASE_URL}${PRODUCTS_URL}/${type}?page=${page}&itemPerPage=${itemPerPage}`)
        .then((items) => {
          this.products = items.data.products
          this.total = items.data.total
          this.page = items.data.page
    ...
    getProductsByCategory(page: number, itemPerPage: number, type: string, category: string) {
      axios
        .get(
          `${BASE_URL}${PRODUCTS_URL}/${type}/${category}?page=${page}&itemPerPage=${itemPerPage}`
        )
        .then((items) => {
          this.products = items.data.products
          this.total = items.data.total
    ...
    async search(searchQuery: string, page: number | null, itemPerPage: number | null) {
      try {
        const items = await axios.get(
          `http://localhost:3030/api/search?term=${searchQuery}&page=${page}&itemPerPage=${itemPerPage}`
        )
        this.products = items.data.products
        this.total = items.data.total
        this.page = items.data.page
